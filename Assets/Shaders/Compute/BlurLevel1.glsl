...
vec4 gaussianBlur(vec2 uv) {
    ...
    vec3 pixel = 0.3125 * DecodeRGBM(texture(u_Input, max(min(uv, maxCoord), minCoord)));

    vec2 offset;
    vec2 blurDir = vec2(1,0) / gl_NumWorkGroups.xy;
    blurDir *= uGlobalTexSize.y * 0.00075;
    offset = blurDir * 1.2857142857142858;

    pixel += 0.328125 *  DecodeRGBM(texture(u_Input, max(min(uv + offset, maxCoord), minCoord)));
    pixel += 0.328125 *  DecodeRGBM(texture(u_Input, max(min(uv - offset, maxCoord), minCoord)));
