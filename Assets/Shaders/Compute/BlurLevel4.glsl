...
vec4 gaussianBlur(vec2 uv) {
    ...
    vec3 pixel = 0.2255859375 * DecodeRGBM(texture2D(u_Input, clamp(uv, minCoord, maxCoord)));
    vec2 offset;
    vec2 blurDir = uBlurDir.xy / gl_NumWorkGroups.xy;
    blurDir *= uGlobalTexSize.y * 0.00075;
    offset = blurDir * 1.3846153846153846;
    pixel += 0.314208984375 * DecodeRGBM(texture(u_Input, clamp(uv + offset, minCoord, maxCoord)));
    pixel += 0.314208984375 * DecodeRGBM(texture(u_Input, clamp(uv - offset, minCoord, maxCoord)));
    offset = blurDir * 3.230769230769231;
    pixel += 0.06982421875 * DecodeRGBM(texture(u_Input, clamp(uv + offset, minCoord, maxCoord)));
    pixel += 0.06982421875 * DecodeRGBM(texture(u_Input, clamp(uv - offset, minCoord, maxCoord)));
    offset = blurDir * 5.076923076923077;
    pixel += 0.003173828125 * DecodeRGBM(texture(u_Input, clamp(uv + offset, minCoord, maxCoord)));
    pixel += 0.003173828125 * DecodeRGBM(texture(u_Input, clamp(uv - offset, minCoord, maxCoord)));
