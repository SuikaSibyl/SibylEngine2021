...
namespace SIByL
{
	...
	void ViewCameraController::CameraState::SetFromTransform(Ref<TransformComponent> t)
	{
	...
	void ViewCameraController::CameraState::UpdateTransform(Ref<TransformComponent> t)
	{
		t->EulerAngles = glm::vec3(pitch, yaw, roll);
		t->Translation = glm::vec3(x, y, z);
	}

	Ref<TransformComponent> ViewCameraController::CameraState::AsTransform()
	{
		Ref<TransformComponent> t = std::make_shared<TransformComponent>();
		t->EulerAngles = glm::vec3(pitch, yaw, roll);
		t->Translation = glm::vec3(x, y, z);
		return t;
	}
