...
#include <vector>
#include <algorithm>
export module Interpolator.AnimationCurve;
...
namespace SIByL::Interpolator
{
	...
	auto AnimationCurve::evaluate(float time) noexcept -> float
	{
		if (time > keyFrames.back().time)// right warp
		{
			switch (preWrapMode)
			{
			case SIByL::Interpolator::WrapMode::CLAMP:
				return keyFrames.back().value;
				break;
			case SIByL::Interpolator::WrapMode::REPEAT:
			{
				...
				break;
			case SIByL::Interpolator::WrapMode::PINGPOMG:
			{
				int passCount = int((time - keyFrames.back().time) / (keyFrames.back().time - keyFrames.front().time));
