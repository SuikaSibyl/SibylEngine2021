...
namespace SIByL::Core
{
	...
	{
		::SE_CORE_DEBUG("╭ BEGIN RUN ALL TESTCASES");
		for (auto iter = tests.begin(); iter != tests.end(); iter++)
		{
			...
			::SE_CORE_TRACE("│ ╭─ TESTCASE: {0}", (*iter)->desc);
			fail_number_local = 0;
			(*iter)->fn();
			::SE_CORE_TRACE("│ ╰─ CHECK FAILURE : {0}", fail_number_local);
		}
		::SE_CORE_DEBUG("╰ FINISH ALL TESTCASES, PASS: {0}, FAIL: {1}", pass_number, fail_number);
	}
